...
function MarcIsoReader(options) {
  ...
  this.options = {
    ...
    // Parse invalid data field if true. Otherwise convert to control field.
    parseInvalidDataField: options.parseInvalidDataField === false ? false : true
  }
}
...
MarcIsoReader.prototype.openFile = function(recordsFile, options) {
  ...
  if (options.hasOwnProperty('parseInvalidDataField')) {
    this.options.parseInvalidDataField = options.parseInvalidDataField;
  }
}
  ...
  fieldStartPos, fieldLength, options)
{
  ...
  // Determine if field should be parsed as control field.
  var isControlField = options.controlFieldRegexp.test(fieldTag);
  if (!isControlField && !options.parseInvalidDataField) {
    if (fieldLength === 2) {
      // Empty field is still data field.
    } else if (fieldLength < 4) {
      // Too short for data field.
      isControlField = true;
    } else if (buffer.readUInt8(fieldStartPos) != 0x1f
      && buffer.readUInt8(fieldStartPos + 1) != 0x1f
      && buffer.readUInt8(fieldStartPos + 2) != 0x1f) {
      // First field is not detected.
      isControlField = true;
    }
  }
