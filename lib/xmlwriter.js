...
MarcXmlWriter.prototype.openFile = function(recordsFile, options) {
  ...
  if (options.hasOwnProperty('bom')) {
    this.options.bom = Boolean(options.bom);
  }
}
...
MarcXmlWriter.prototype.open = function(recordsFileName) {
  ...
  fs.open(recordsFileName, flags, mode, function(err, recordsFile) {
    ...
    var header = (self.options.bom ? '\uFEFF' : '')
      + '<?xml version="1.0" encoding="'
      + (self.options.encoding || 'utf-8') + '" ?>\n';
    if (self.options.format == 'MARC21') {
    ...
    var buffer = self.options.encoding ?
      ...
      : bufferWrapper.from(header, 'utf-8');
...
MarcXmlWriter.prototype.openSync = function(recordsFileName, options) {
  ...
  var header = (this.options.bom ? '\ufeff' : '')
    + '<?xml version="1.0" encoding="'
    + (this.options.encoding || 'utf-8') + '" ?>\n';
  if (this.options.format == 'MARC21') {
    header += '<collection xmlns="http://www.loc.gov/MARC21/slim">\n';
  } else {
    header += '<collection>\n';
  }
  var buffer = this.options.encoding ?
    iconv.encode(header, this.options.encoding)
    : bufferWrapper.from(header, 'utf-8');
...
MarcXmlWriter.prototype.close = function(callback) {
  ...
  if (self.recordsFile !== null) {
    ...
    var buffer = self.options.encoding ?
      ...
      : bufferWrapper.from(footer, 'utf-8');
...
MarcXmlWriter.prototype.write = function(record, callback) {
  ...
  var buffer = self.options.encoding ?
    ...
    : bufferWrapper.from(marcXmlRecord, 'utf-8');
  fs.write(self.recordsFile, buffer, 0, buffer.length, null,
    function(err, written) {
      self.position += written;
      callback(err);
 ...
 */
MarcXmlWriter.prototype.getPosition = function() {
