 ...
 * Adds subfields.
 */
MarcDataField.prototype.addSubfields = function(index, subfields) {
  if (index instanceof Array) {
    Array.prototype.push.apply(this.subfields, index);
  } else if (subfields instanceof Array) {
    Array.prototype.splice.apply(this.subfields, [index, 0].concat(subfields));
  } else {
    throw new Error('invalid type of subfields');
  }
}
 ...
 */
MarcDataField.prototype.addNonEmptySubfield = function(index, subfield) {
  if (index instanceof MarcSubfield) {
    if (!index.empty()) {
      this.subfields.push(index);
    }
